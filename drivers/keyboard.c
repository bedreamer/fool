...
static _u32 keymap[NR_SCAN_CODES * MAP_COLS] = 
{
	...
	/* 0x51 - PgDown	*/	PAD_PGDOWN,'3',		PAGEDOWN,
...
void initializekeyboard(void)
{
	if (0xEE==_8042detect())
	{
		//printk("detect Intel 8042 keyboard contraller OK.");
		if (hwregistehandle(KEYBOARD_HWINT1,
			keyboardname,keyboardhandle)) {
			//printk("registe keyboard irq %d success,procedure:0x%x.",
			//KEYBOARD_HWINT1,keyboardhandle);
			_8259Asti(KEYBOARD_HWINT1);
		} else {
			//printk("registe keyboard irq %d faile.");
		}
	}
}
